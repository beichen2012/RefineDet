...
def AddExtraLayers(net, use_batchnorm=True, arm_source_layers=[], normalizations=[], lr_mult=1):
    ...
    for index, layer in enumerate(arm_source_layers):
        ...
        ConvBNLayer(net, from_layer, out_layer, use_batchnorm, use_relu, 96, 3, 1, 1, lr_mult=lr_mult)
        ...
        if num_p == 6:
            ...
            from_layer = out_layer
            out_layer = "TL{}_{}".format(num_p, 2)
            ConvBNLayer(net, from_layer, out_layer, use_batchnorm, False, 96, 3, 1, 1, lr_mult=lr_mult)

            from_layer = "P{}".format(num_p+1)
            out_layer = "P{}-up".format(num_p+1)
            DeconvBNLayer(net, from_layer, out_layer, use_batchnorm, False, 96, 2, 0, 2, lr_mult=lr_mult)
...
train_data = "/homec/wyj/dataset/VOCdevkit/RefineDet/lmdb"
# The database file for testing data. Created by data/VOC0712/create_data.sh
test_data = "/homec/wyj/dataset/VOCdevkit/RefineDet/lmdb"
# Specify the batch sampler.
resize_width = 320
resize_height = 320
resize = "{}x{}".format(resize_width, resize_height)

# batch_sampler : data augment
#    scale: area ratio of random patch with ori pic
#    jaccard_overlap: IoU of random bbox with ori ground truth
#    trials: iter times
#    random,0.1,0.3,0.5,0.7,0.9,1
batch_sampler = [
        {
                'sampler': {
                        },
...
job_name = "MobileNetV2_VOC2012_{}".format(resize)
# The name of the model. Modify it if you want.
model_name = "{}".format(job_name)

# Directory which stores the model .prototxt file.
# Directory which stores the model .prototxt file.
save_dir = "models/MobileNetV2/VOC2012/{}".format(job_name)
# Directory which stores the snapshot of models.
snapshot_dir = "models/MobileNetV2/VOC2012/{}".format(job_name)
# Directory which stores the job script and log file.
job_dir = "jobs/MobileNetV2/VOC2012/{}".format(job_name)
# Directory which stores the detection results.
output_result_dir = "{}/dataset/VOCdevkit/RefineDet/results/{}".format(os.environ['HOME'], job_name)
...
# fc7 ==> 10 x 10
# conv6_2 ==> 5 x 5
arm_source_layers = ['conv4_3/sum', 'conv6_3/sum', 'conv7_3/sum', 'conv8_1/out/pw/scale']
...
# MobileNetV2Body(net, from_layer='data')
MobileNetV2Body(net, from_layer='data', alpha=0.75, c=5)
...
with open(job_file, 'w') as f:
  f.write('cd {}\n'.format(caffe_root))
  f.write('/home/ss/wzl/github/RefineDet/build/tools/caffe train \\\n')
  f.write('--solver="{}" \\\n'.format(solver_file))
